...
import language.ref.field.intField.IntERef;

import java.util.HashMap;

/** IntE represents an integer language.obj.field on edges. */
public class IntE extends IntField<BoolE> {
    ...
    public IntE(int n, Border border) {
        ...
        for (int i = 0; i <= n; i++) this.bits[i] = BoolERef.of(BoolE.zeroes(border));
    }
    ...
    public static IntE of(int value, int n, Border border) {
        ...
        for (int i = 0; value != 0 && value != Integer.MIN_VALUE; i++) {
            ...
            intE.bits[n - i] = (value & 1) == 0 ? BoolERef.of(BoolE.zeroes(border)) : BoolERef.of(BoolE.ones(border));
            value = value >> 1;
        }
        intE.bits[0] = value >>> 31 == 0 ? BoolERef.of(BoolE.zeroes(border)) : BoolERef.of(BoolE.ones(border));
        return intE;
    }

    public static IntE of(BoolERef boolE, int n) {
        IntE intE = new IntE(n, boolE.get().border);
        for (int i = 1; i < n; i++) intE.bits[i] = BoolERef.of(BoolE.zeroes(boolE.get().border));
    ...
    protected static void lShift(IntE a, IntE res, int k, int n) {
        ...
        for (int i = n - k + 1; i <= n; i++) res.bits[i] = BoolERef.of(BoolE.zeroes(a.border));
    }
    ...
    protected static void rShift(IntE a, IntE res, int k, int n) {
        ...
        for (int i = 0; i < k; i++) res.bits[i] = BoolERef.of(BoolE.zeroes(a.border));
    }
}
